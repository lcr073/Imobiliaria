...
// Valida no banco as informações recebidas
   include "include/db/connect.php";
    ...
    $stmt = $dbh->prepare("SELECT  id FROM tbl_user WHERE nome= :NOME OR email= :EMAIL;");
     ...
     // Sai e da erro
      ...
      // Verifica se tudo veio preenchido
        ...
        if(isset($obj['nome']) AND isset($obj['usuario']) AND isset($obj['email']) AND isset($obj['senha'])){
            ...
            // Array associativo com as opções desejadas
    //        $option = [
     //           'salt' => "Vasco Frango não é de calabresa";
     //       ];

    //        $senha = password_hash($obj['senha'], PASSWORD_BCRYPT, $options);
$senha = "0i";
            // Realiza insercao DB
